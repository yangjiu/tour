...
import com.joysus.chat.common.utils.ImageLoadUtils;
...
public class ChattingListAdapter extends BaseAdapter {
    ...
    public int getItemViewType(int position) {
        ...
        ImMsgBean msgBean = mData.get(position);
        if (msgBean.getSenderType() == ImMsgBean.CHAT_SENDER_ME) {
            if (msgBean.getMsgType() == ImMsgBean.CHAT_MSGTYPE_TEXT)
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        switch (type) {
            ...
            case VIEW_TYPE_RIGHT_TEXT:
                ViewHolderRightText rholder;
                if (convertView == null) {
                    rholder = new ViewHolderRightText();
                    holderView = mInflater.inflate(R.layout.listitem_cha_right_text, null);
                    holderView.setFocusable(true);
                    rholder.iv_avatar = (ImageView) holderView.findViewById(R.id.iv_avatar);
                    rholder.tv_content = (TextView) holderView.findViewById(R.id.tv_content);
                    holderView.setTag(rholder);
                    convertView = holderView;
                } else {
                    rholder = (ViewHolderRightText) convertView.getTag();
                }
                disPlayRightTextView(position, convertView, rholder, bean);
                break;
            case VIEW_TYPE_RIGHT_IMAGE:
                ViewHolderRightImage rimageHolder;
                if (convertView == null) {
                    rimageHolder = new ViewHolderRightImage();
                    holderView = mInflater.inflate(R.layout.listitem_chat_right_image, null);
                    holderView.setFocusable(true);
                    rimageHolder.iv_avatar = (ImageView) holderView.findViewById(R.id.iv_avatar);
                    rimageHolder.iv_image = (ImageView) holderView.findViewById(R.id.iv_image);
                    holderView.setTag(rimageHolder);
                    convertView = holderView;
                } else {
                    rimageHolder = (ViewHolderRightImage) convertView.getTag();
                }
                disPlayRightImageView(position, convertView, rimageHolder, bean);
                break;
            default:
