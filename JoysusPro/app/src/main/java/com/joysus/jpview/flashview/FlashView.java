package com.joysus.jpview.flashview;
...
import com.joysus.R;
import com.joysus.jpview.flashview.effect.AccordionTransformer;
import com.joysus.jpview.flashview.effect.CubeTransformer;
import com.joysus.jpview.flashview.effect.DefaultTransformer;
import com.joysus.jpview.flashview.effect.DepthPageTransformer;
import com.joysus.jpview.flashview.effect.InRightDownTransformer;
import com.joysus.jpview.flashview.effect.InRightUpTransformer;
import com.joysus.jpview.flashview.effect.RotateTransformer;
import com.joysus.jpview.flashview.effect.ZoomOutPageTransformer;
import com.joysus.jpview.flashview.listener.FlashViewListener;
import com.joysus.util.ImageLoaderTools;
...
public class FlashView extends FrameLayout {
	...
	private final String TAG = FlashView.class.getSimpleName();
	...
	public void setImageUris(List<String> imageuris) {
		...
		if (imageuris.size() <= 0)// 如果得到的图片张数为0，则增加一张默认的图片
		{
			...
			imageUris.add("drawable://" + R.drawable.ic_launcher);
		} else {
			if (imageuris.size() == 2) {
				isTwo = true;
		...
		Logger.LOGD(TAG, "测试：：：imageUris.size()" + imageUris.size());
...
//			}
	...
	private class MyPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(View container, int position) {
			...
			Logger.LOGD(TAG, "position111:::" + position);
			if (isTwo) {
				pos = position % 2;
			} else {
				pos = position;
			...
			if (vp != null) {
				Logger.LOGD(TAG, "JJJJJ" + pos);
				ViewPager pager = (ViewPager) vp;
				Logger.LOGD(TAG,
						"pager.getChildCount():" + pager.getChildCount());
	...
	public void FlashViewResume() {
		mhandler.sendEmptyMessageDelayed(ImageHandler.MSG_UPDATE_IMAGE,
				ImageHandler.MSG_DELAY);
	}

	public void FlashViewPause() {
